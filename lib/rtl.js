...
var DQNAgent = function(env, opt) {
  ...
  // target network: copy of online net updated every N steps for stable Q targets
  this.target_update_every = getopt(opt, 'target_update_every', 100);

  // epsilon decay: agent gradually shifts from exploration to exploitation
  this.epsilon_min = getopt(opt, 'epsilon_min', 0.05);
  this.epsilon_decay = getopt(opt, 'epsilon_decay', 0.9995);
...
DQNAgent.prototype = {
  reset: function() {
    ...
    // target network: a periodically-synced copy used to compute stable Bellman targets
    this.targetNet = R.copyNet(this.net);

    this.exp = []; // experience replay buffer
    this.expi = 0; // circular buffer write pointer
  ...
  toJSON: function() {
    ...
    j.targetNet = R.netToJSON(this.targetNet);
  ...
  fromJSON: function(j) {
    ...
    this.targetNet = j.targetNet ? R.netFromJSON(j.targetNet) : R.copyNet(this.net);
    if(typeof j.epsilon !== 'undefined') { this.epsilon = j.epsilon; }
  },
  ...
  learn: function(r1) {
    ...
    if(!(this.r0 == null) && this.alpha > 0) {
      ...
      if(this.t % this.experience_add_every === 0) {
        ...
        this.expi += 1;
        if(this.expi >= this.experience_size) { this.expi = 0; } // roll over when buffer is full
      }
      this.t += 1;
      ...
      if(this.t % this.target_update_every === 0) {
        this.targetNet = R.copyNet(this.net);
      }
  ...
  learnFromTuple: function(s0, a0, r0, s1, a1) {
    ...
    // compute the target Q value using the frozen target network for stability
    var tmat = this.forwardQ(this.targetNet, s1, false);
